...
class AuthManager extends Manager {
    ...
    public function checkAccess($operation, $uid,$strtolower =true, $params=array()) {
        ...
        $removeAuth = $remove->getRemoveAuth($uid,$operation);
        if($removeAuth){
            return false;
        }
...
//     	$redis = new Redisdb();
//     	$key = 'auth_username';
//     	if ($redis->exists($key, array($uid))) {
//     		return json_decode($redis->get($key, array($uid)), true);
//     	}
//         $output = yii::app()->curl->get("http://api.speed.meilishuo.com/user/show?token=e98cfc1a4f23ae1699919c505ae2ba04&id=".$uid);
//         $results = (array)json_decode($output['body']);
//         $results = $results['data'];
//         $depart = $results->name;
//         $redis->set($key, array($uid), json_encode($depart));
//         return $depart;
//     }
    	...
    	$variable = array($role=>1);
    	 ...
    	 if ($role_admin) {
    	 	foreach ($role_admin as $aval) {
    	 		$tmp = NewCommonManager::getUsernameByuid($aval);
    	 		if ($tmp) {
    	 			$usename[] = $tmp;
    	 		}
    	 	}
